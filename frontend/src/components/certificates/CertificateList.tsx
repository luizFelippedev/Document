...
export const CertificateList = () => {
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      try {
        ...
        showToast('error', error.response?.data?.message || 'Failed to load certificates');
      } finally {
        setLoading(false);
      }
    };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast('error', error.response?.data?.message || 'Failed to delete certificate');
    }
  };
              ...
              <div className="text-center py-12">
                <div className="flex justify-center mb-4">
                  <div className="p-3 bg-gray-100 dark:bg-gray-800 rounded-full">
                    <Award size={24} className="text-gray-500 dark:text-gray-400" />
                  </div>
                </div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">No certificates found</h3>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className={cn(
                        ...
                        getCertificateColor(certificate.title, certificate.issuer)
                      )}>
                      ...
                      <div className="p-4 flex-1 flex flex-col">
                        ...
                        <div className="mb-4">
                          <div className="flex flex-wrap gap-1.5">
                            {certificate.skills.slice(0, 3).map((skill) => (
                              <Badge key={skill} text={skill} variant="outline" size="sm" />
                            ))}
                            {certificate.skills.length > 3 && (
                              <Badge text={`+${certificate.skills.length - 3}`} variant="outline" size="sm" />
                            )}
                          </div>
                        </div>
                        ...
                        {certificate.credentialId && (
                          <div className="flex items-center mb-2 text-xs text-gray-500 dark:text-gray-400">
                            <span className="font-medium mr-1">ID:</span> {certificate.credentialId}
                          </div>
                        )}
