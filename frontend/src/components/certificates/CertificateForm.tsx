...
export const CertificateForm = ({ id, initialData }: CertificateFormProps) => {
  ...
  const handleFileUpload = (files: File[]) => {
    if (files.length > 0) {
      ...
      if (!ALLOWED_FILE_TYPES.includes(file.type)) {
        showToast('error', 'Please upload a PDF or image file (JPEG, PNG, WebP)');
        return;
      }
  ...
  const onSubmit = async (data: CertificateFormValues) => {
    try {
      ...
      showToast(
        'error',
        error.response?.data?.message || 'An error occurred while saving the certificate'
      );
    } finally {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col justify-center">
                  <div className="p-4 border border-blue-200 dark:border-blue-800 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                    <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-2">Tips for Certificate Verification</h3>
                    <ul className="text-xs text-blue-700 dark:text-blue-400 space-y-1">
                      <li className="flex items-start">
                        <span className="mr-1.5 mt-0.5">•</span>
                        <span>Include the credential ID if available to make verification easier</span>
                      </li>
                      <li className="flex items-start">
                        <span className="mr-1.5 mt-0.5">•</span>
                        <span>Add the verification URL where others can validate the certificate</span>
                      </li>
                      <li className="flex items-start">
                        <span className="mr-1.5 mt-0.5">•</span>
                        <span>Upload a clear, legible copy of your certificate</span>
                      </li>
                      <li className="flex items-start">
                        <span className="mr-1.5 mt-0.5">•</span>
                        <span>Ensure expiration date is accurate to maintain credibility</span>
                      </li>
                    </ul>
                  </div>
                </div>
