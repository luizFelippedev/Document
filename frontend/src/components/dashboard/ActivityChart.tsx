...
import { BarChart2, TrendingUp, TrendingDown, Download, Filter } from 'lucide-react';
...
export const ActivityChart = ({
  ...
  const handleDownload = () => {
    ...
    const rows = currentData.map(item => Object.values(item).join(',')).join('\n');
    ...
    if (link) {
      ...
      link.setAttribute('download', `activity_data_${new Date().toISOString().slice(0, 10)}.csv`);
      link.click();
    }
  };
      ...
      return (
        <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-700 rounded-md shadow-md">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-white">
            Count: <span className="text-blue-600 dark:text-blue-400">{payload[0].value}</span>
          </p>
        </div>
      );
    }
            ...
            <div className="flex items-center">
              {trend > 0 ? (
                <>
                  ...
                  <span className="text-green-500 font-medium">+{trend.toFixed(1)}%</span>
                </>
              ) : trend < 0 ? (
                <>
                  <TrendingDown size={16} className="text-red-500 mr-1" />
                  <span className="text-red-500 font-medium">{trend.toFixed(1)}%</span>
                </>
              ) : (
        ...
        <div className="sm:hidden">
          <button
            ...
            onClick={() => {
              ...
              const types: ('line' | 'area' | 'bar')[] = ['line', 'area', 'bar'];
