...
import {
  Trash2, ChevronLeft, ChevronRight, Upload, X, GitHub, Globe, AlertCircle,
  Clock, CheckCircle, Save, ArrowLeft, Eye, Calendar, Link as LinkIcon
} from 'lucide-react';
...
export const ProjectForm = ({ id, initialData }: ProjectFormProps) => {
  ...
  const handleThumbnailUpload = (files: File[]) => {
    if (files.length > 0) {
      ...
      if (!ALLOWED_FILE_TYPES.includes(file.type)) {
        showToast('error', 'Please upload an image file (JPEG, PNG, WebP, or GIF)');
        return;
      }
  ...
  const handleImagesUpload = (files: File[]) => {
    if (files.length > 0) {
      ...
      const validFiles = files.filter(file => {
        if (!ALLOWED_FILE_TYPES.includes(file.type)) {
          showToast('error', 'Please upload image files only (JPEG, PNG, WebP, or GIF)');
          return false;
        }
  ...
  const toggleSkill = (skill: string) => {
    ...
    if (currentSkills.includes(skill)) {
      setValue('skills', currentSkills.filter(s => s !== skill), { shouldDirty: true });
    } else {
  ...
  const onSubmit = async (data: ProjectFormValues) => {
    try {
      ...
      if (initialData?.images) {
        const deletedImages = initialData.images.filter(url => !imagePreviewUrls.includes(url));
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <Switch
                    id="completed"
                    checked={completed}
                    onChange={(checked) => setValue('completed', checked, { shouldDirty: true })}
                  ...
                  <div className="mt-8 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                      <div>
                        ...
                        <p className="text-base text-gray-900 dark:text-white">{watch('title')}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Category</p>
                        <p className="text-base text-gray-900 dark:text-white">
                          {watch('category') ? watch('category').replace('-', ' ').replace(/\b\w/g, char => char.toUpperCase()) : 'Not specified'}
                        </p>
                      </div>
                      ...
                      {watch('startDate') && (
                        <div>
                          ...
                          <p className="text-base text-gray-900 dark:text-white flex items-center">
                            ...
                            {format(new Date(watch('startDate')), 'MMMM d, yyyy')}
                          </p>
                        </div>
                      )}
                      ...
                      {watch('demoUrl') && (
                        <div>
                          <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Demo URL</p>
                          <a 
                            href={watch('demoUrl')} 
                            target="_blank" 
