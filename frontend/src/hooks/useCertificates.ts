...
export const useCertificates = () => {
  ...
  const createCertificate = useCallback(
    async (certificateData: FormData) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateCertificate = useCallback(
    async (id: string, certificateData: FormData) => {
      try {
        setLoading(true);
        setError(null);

        const data = await certificateService.updateCertificate(
          id,
          certificateData,
        );
  ...
  const filterCertificates = useCallback(
    async (filters: CertificateFilter) => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (filters.category) {
          filteredCerts = filteredCerts.filter(
            (cert) => cert.category === filters.category,
          );
        }
