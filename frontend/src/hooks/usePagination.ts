...
export const usePagination = ({
  ...
  const [currentPage, setCurrentPage] = useState(getInitialValue(pageParam, initialPage));
  const [pageSize, setCurrentPageSize] = useState(getInitialValue(pageSizeParam, initialPageSize));
  ...
  const setPageSize = useCallback(
    ...
    [totalItems, currentPage, useQueryParams, searchParams, pageSizeParam, pageParam, router]
  );
  ...
  const pageNumbers = useMemo(() => {
    ...
    const rangeEnd = Math.min(totalPages - 1, normalizedCurrentPage + siblingCount);
