...
export const useProjects = () => {
  ...
  // Create project
  const createProject = useCallback(async (projectData: FormData) => {
    try {
      setLoading(true);
      setError(null);
  ...
  // Update project
  const updateProject = useCallback(async (id: string, projectData: FormData) => {
    try {
      setLoading(true);
      setError(null);
  ...
  // Filter projects
  const filterProjects = useCallback(async (filters: ProjectFilter) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (filters.search) {
        ...
        filteredProjs = filteredProjs.filter(proj => 
          ...
          proj.skills.some(skill => skill.toLowerCase().includes(searchTerm))
        );
      }
      ...
      if (filters.completed !== undefined) {
        filteredProjs = filteredProjs.filter(proj => 
          proj.completed === filters.completed
        );
      }
      ...
      if (filters.sortBy) {
        switch (filters.sortBy) {
          case 'newest':
            filteredProjs.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            break;
          case 'oldest':
            filteredProjs.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            break;
          case 'a-z':
